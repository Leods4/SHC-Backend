...
GET      | /certificados/export          | Exporta dados para sistema externo
-------------------------------------------------------------------------------------
...
#### Filtros de busca GET /certificados/export (Exportação Externa)

Os mesmos filtros da listagem geral podem ser aplicados para a exportação de dados para sistemas externos:

status        → ENTREGUE, APROVADO, REPROVADO, APROVADO_COM_RESSALVAS  
aluno_id      → filtra por aluno específico  
categoria_id  → filtra por categoria de certificado  
data_inicio   → YYYY-MM-DD (data de emissão)  
data_fim      → YYYY-MM-DD (data de emissão)  

**Nota de Segurança:** A exportação respeita as regras de permissão do usuário que gerou o Token (Alunos só exportam os próprios dados; Coordenadores apenas do seu curso; Admin/Secretaria exportam tudo). Para integrações externas completas, recomenda-se gerar um token de acesso associado a um usuário Administrador.

#### Payload de Resposta — Exportação (GET /certificados/export)
Retorna uma coleção JSON formatada idêntica à listagem, pronta para consumo externo:
  ...
  "data": [
    {
      ...
      "nome_certificado": "Curso Laravel Avançado",
      "instituicao": "FMP",
      "carga_horaria_solicitada": 40,
      "status": "APROVADO",
      "data_emissao": "2025-10-15",
      "data_emissao_formatada": "15/10/2025",
      "created_at": "2025-10-16 14:30:00",
      "arquivo_url": "http://localhost:8000/storage/certificados/arquivo.pdf",
      "horas_validadas": 40,
      "observacao": "Validado com sucesso",
      "data_validacao": "17/10/2025 09:00",
      "aluno": {
         "id": 42,
         "nome": "João Silva"
      },
...
Agora, além dos dados de avaliação, o coordenador pode corrigir as informações enviadas originalmente pelo aluno. 

**Todos os campos abaixo (exceto o `status`) são opcionais na requisição**. Envie os campos referentes ao certificado (`categoria_id`, `nome_certificado`, etc.) apenas se desejar alterá-los.

```json
{
  "status": "APROVADO",
  "horas_validadas": 10,
  "observacao": "Validação ok. O nome da instituição foi corrigido.",
  "categoria_id": 2,
  "nome_certificado": "Curso Avançado de PHP",
  "instituicao": "Udemy",
